...
class DuplicateChecker:
    ...
    def __init__(self, 
                 ...
                 ahash_threshold: int = 8,  # 降低aHash阈值至8，进一步减少误判
        ...
        Args:
            phash_threshold: pHash感知哈希相似度阈值，默认为8
            dhash_threshold: dHash感知哈希相似度阈值，默认为8
            ahash_threshold: aHash感知哈希相似度阈值，默认为8（降低以进一步减少误判）
    ...
    def check_exact_duplicates(self, image_files: List[str]) -> Dict[str, List[str]]:
        ...
        print(f"正在计算 {len(image_files)} 个文件的哈希值...")
    ...
    def check_pure_color_images(self, image_files: List[str]) -> List[str]:
        ...
        for i, file_path in enumerate(image_files):
            ...
            try:
                ...
                print(f"检测纯色图片时出错: {file_path}, 错误: {str(e)}")
    ...
    def check_perceptual_duplicates(self, image_files: List[str]) -> List[Dict[str, Any]]:
        ...
        """
        # 计算所有图片的感知哈希
        print(f"正在计算 {len(image_files)} 个文件的感知哈希...")
        ...
        for i, file_path in enumerate(image_files):
            ...
            try:
                ...
                print(f"计算感知哈希时出错: {file_path}, 错误: {str(e)}")
        ...
        for i, file1 in enumerate(file_hashes):
            ...
            if i % 100 == 0 and i > 0:
                print(f"已处理 {i}/{len(file_hashes)} 个文件...")
            ...
            for file2 in file_hashes:
                ...
                if dhash_distance < self.dhash_threshold:
                    similar_files.append(file2['path'])
                    reason = "dHash相似"
                elif phash_distance < self.phash_threshold:
                    similar_files.append(file2['path'])
                    reason = "pHash相似"
                elif ahash_distance < self.ahash_threshold:  # 使用更高的aHash阈值
                    similar_files.append(file2['path'])
                    reason = "aHash相似"
            ...
            if similar_files:
                similar_group = {
                    'reason': reason,
                    'files': [file1['path']] + similar_files
                }
    ...
    def check_duplicates(self, directory: str, recursive: bool = True) -> Dict[str, Any]:
        ...
        print(f"总处理图片数: {self.stats['total_images']}")
        print(f"重复图片组数: {self.stats['duplicate_groups']}")
        print(f"重复图片总数: {self.stats['duplicate_images']}")
        if self.pure_color_images:
            print(f"纯色图片数: {self.stats['pure_color_images']}")
